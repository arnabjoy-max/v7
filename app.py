...
# ══════════════════════════════════════════════════════════════════
    ...
    # Date range for the week: Monday → Sunday
    ...
    if st.button("＋  Add Day", use_container_width=True):
        if len(days) < 7:
            new_day = _day_empty()
            # Default new day date to next available day in the week
            used = {d.get("day_date") for d in days if d.get("day_date")}
            for offset in range(7):
                candidate = sd + timedelta(days=offset)
                if candidate not in used:
                    new_day["day_date"] = candidate
                    break
            days.append(new_day)
            st.session_state.days = days
            st.rerun()
        else:
            st.warning("Maximum 7 days per FIR report.")
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    # ── Totals summary ────────────────────────────────────────────
    if days:
        ts = sum((d.get("straight") or 0) for d in days)
        to = sum((d.get("overtime") or 0) for d in days)
        tt = sum((d.get("travel_time") or 0) for d in days)
        total = ts + to + tt + sum((d.get("doubletime") or 0) for d in days)
        st.markdown(f"""
    ...
    if not days:
        ...
        <div style="text-align:center;padding:40px 20px;color:#9CA3AF;">
          ...
          <div style="font-weight:600;font-size:15px;color:#374151;margin-bottom:6px;">No days added yet</div>
          <div style="font-size:13px;">Tap <b style="color:#1A1A2E;">＋ Add Day</b> to start logging</div>
        </div>""", unsafe_allow_html=True)
    else:
        to_delete = None
        for i, d in enumerate(days):
            # ── Day date header ───────────────────────────────────
            day_date = d.get("day_date") or sd
            ...
            # ── Inputs inside continuation of card ────────────────
            with st.container():
                st.markdown('<div style="margin:0 12px;background:#FFFFFF;border:1.5px solid #E0E3E8;border-top:none;border-radius:0 0 12px 12px;padding:2px 14px 14px;">', unsafe_allow_html=True)

                # Date picker — any date within the week
                new_date = st.date_input(
                    ...
                    max_value=week_end,
                    key=f"date_{i}")
                st.session_state.days[i]["day_date"] = new_date
                ...
                with h1:
                    v = st.number_input("S",  min_value=0.0, max_value=24.0, value=None,
                        step=0.5, key=f"s_{i}",  format="%.1f")
                    st.session_state.days[i]["straight"] = v
                with h2:
                    v = st.number_input("OT", min_value=0.0, max_value=24.0, value=None,
                        step=0.5, key=f"ot_{i}", format="%.1f")
                    st.session_state.days[i]["overtime"] = v
                with h3:
                    v = st.number_input("DT", min_value=0.0, max_value=24.0, value=None,
                        step=0.5, key=f"dt_{i}", format="%.1f")
                    st.session_state.days[i]["doubletime"] = v
                with h4:
                    v = st.number_input("TT", min_value=0.0, max_value=24.0, value=None,
                        step=0.5, key=f"tt_{i}", format="%.1f")
                    st.session_state.days[i]["travel_time"] = v
                with h5:
                    v = st.number_input("W",  min_value=0.0, max_value=24.0, value=None,
                        step=0.5, key=f"w_{i}",  format="%.1f")
                    st.session_state.days[i]["working"] = v

                # Wait + Delete
                w_col, del_col = st.columns([3,1])
                with w_col:
                    v = st.number_input("Wait", min_value=0.0, max_value=24.0, value=None,
                        step=0.5, key=f"wt_{i}", format="%.1f")
        ...
        if to_delete is not None:
            st.session_state.days.pop(to_delete)
            st.rerun()
...
# ══════════════════════════════════════════════════════════════════
    ...
    if not st.session_state.generated_bytes:
        ...
        recipients_raw = st.text_area("Recipients (one per line)",
                                       placeholder="manager@fanuc.com", height=90)
        ...
        if st.button("📤  SEND EMAIL", use_container_width=True):
            if not all([smtp_host, smtp_user, smtp_pass, recipients_raw]):
                ...
                body = (f"FIR attached — Project #{st.session_state.get('project_num','')}\n\n"
                        ...
                        f"ID: {st.session_state.get('emp_no', st.session_state.get('last_name',''))}\n"
...
<div style="text-align:center;padding:28px 0 10px;color:#9CA3AF;font-size:10px;letter-spacing:1px;">
  FANUC AMERICA CORPORATION · FIR SYSTEM v7.0<br>
